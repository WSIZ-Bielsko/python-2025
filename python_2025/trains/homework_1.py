...
def reverse_train(train: TrainInfo) -> TrainInfo:
    return TrainInfo(
        id=train.id,
        ...
        total_time_minutes=train.total_time_minutes,
        departure_time=train.arrival_time,
        arrival_time=train.departure_time,
    )


def get_trains_from(city_prefix: str, all_trains: list[TrainInfo], reverse=False) -> list[TrainInfo]:
    ...
    for t in all_trains:
        ...
        if reverse:
            g = reverse_train(t)
        if g.source.lower().startswith(city_prefix):
            res.append(t)
    return res
...
def test_get_trains_on_destination():
    t1 = TrainInfo(id=1, train_name='a', train_number='01', source='Bielsko-Biała', destination='Warszawa', distance=250,
                   total_time_minutes=240, departure_time=time(12, 0, 0), arrival_time=time(15, 0, 0))
    t2 = TrainInfo(id=2, train_name='b', train_number='01', source='Wrocław', destination='Warszawa', distance=250,
                   total_time_minutes=240, departure_time=time(12, 0, 0), arrival_time=time(15, 0, 0))
    ...
    res = get_trains_from('War', [t1, t2, t3], reverse=True)
    print(res)
